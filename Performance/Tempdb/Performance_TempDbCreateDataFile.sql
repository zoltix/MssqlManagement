...
SET NOCOUNT ON
GO
PRINT '-- Instance name: '+ @@servername + ' ;/* Version: ' + @@version + ' */'
...
BEGIN
    ...
    SELECT @SQL = 'ALTER DATABASE [tempdb] ADD FILE (NAME = N''tempdev_0'+CAST (@tempdb_files_count +1 AS VARCHAR (5))+''',FILENAME = N'''+ @new_files_Location + 'tempdev_0'+CAST (@tempdb_files_count +1 AS VARCHAR(5)) +'.ndf'',SIZE = '+CAST(@new_tempdbdev_size_MB AS VARCHAR(15)) +'MB, FILEGROWTH = 0 )
GO'
    PRINT @SQL
...
--DBCC SHRINKFILE (tempdev_03, EMPTYFILE); -- to empty "tempdev12" data file
--GO
--DBCC SHRINKFILE (tempdev_03, EMPTYFILE); -- to empty "tempdev12" data file
--Go
...
--REMOVE FILE tempdev_02; --to delete "tempdev12" data file